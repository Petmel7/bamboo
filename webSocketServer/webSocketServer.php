...
$ws_worker->onMessage = function ($connection, $data) use (&$connectedUsers, &$connectionUserMap) {
    ...
    if (isset($message['action']) && $message['action'] === 'delete' && isset($message['message_id'])) {
    ...
    } elseif (isset($message['action']) && $message['action'] === 'add_image' && isset($message['sender_id'], $message['recipient_id'], $message['image_url'])) {
        $senderId = $message['sender_id'];
        $recipientId = $message['recipient_id'];
        $imageUrl = $message['image_url'];
        ...
        try {
            ...
            // Logging the response data to understand the flow
            echo "Broadcasting update to clients: " . json_encode($responseData) . "\n";
